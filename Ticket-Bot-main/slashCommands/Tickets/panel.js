...
module.exports = {
    ...
    async execute(interaction, client) {
      await interaction.deferReply({ ephemeral: true });
        ...
        if(config.TicketSettings.SelectMenu === false) interaction.channel.send({ embeds: [ticketEmbed], components: [row] });
        interaction.editReply({ content: `Successfully sent the ticket panel to this channel!`, ephemeral: true })
        if(config.TicketSettings.SelectMenu) interaction.channel.send({ embeds: [ticketEmbed], components: [sRow] }).then(async function(msg) {
